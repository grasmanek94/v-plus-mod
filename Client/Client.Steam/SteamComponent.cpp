...
void SteamComponent::InitializePresence()
{
	...
	if (configAdded)
	{
		...
		wchar_t _commandLine[MAX_PATH];
		swprintf_s(_commandLine, MAX_PATH, L"\"%s\" -steamchild:%d", ourPath, GetCurrentProcessId());

		std::wstring commandLine = _commandLine;
...
bool SteamComponent::RunPresenceDummy()
{
	...
	if (commandLineMatch)
	{
		...
		if (processHandle != INVALID_HANDLE_VALUE)
		{
			...
			LOG_PRINT("waiting for process to exit...\n");
